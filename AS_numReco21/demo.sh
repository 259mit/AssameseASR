...
train_nj=4
# Single test file from single speaker
#decode_nj=4
decode_nj=1
...
#./create_bigram_lm.sh
touch data/test/cmvn.scp
rm    data/test/cmvn.scp
#================================================
# record demo speech (8sec duration) and create neccessary files
#================================================
rm demo_output.txt
touch demo_output.txt

cd data/test
[ -f serialNum.txt ] || echo "0" > serialNum.txt
num=`cat serialNum.txt`
((num++))
rm serialNum.txt text trans utt spk utt2spk spk2utt
echo $num > serialNum.txt
read -p "Press ENTER to start recording "
rec -r 22050 -b 16 -c 1 demo$num.wav trim 0 8

echo -e "demo$num\t$(pwd)/demo$num.wav"	> wav.scp
echo -e "demo"			> spk
echo -e "demo$num\tkemerii" 	> text
  perl -nae 'print "@F[1..$#F]\n";' text  >  trans
  perl -nae 'print $F[0]' text	> utt 
  paste utt spk 		> utt2spk
  ../../utils/utt2spk_to_spk2utt.pl utt2spk > spk2utt
cd ../../
date

date >> demo_output.txt
echo "" >> demo_output.txt
#===============================================================
...
#  for x in train test ; do 
  for x in test ; do 
	...
	#steps/make_mfcc_nj1.sh --cmd "$train_cmd"  data/$x exp/make_mfcc/$x $mfccdir || exit 1;
...
egrep '^demo' exp/mono/$decode_dir1/log/decode.1.log | cut -c 7- >> demo_output.txt
echo "" >> demo_output.txt; 
#echo "mono" >> demo_output.txt
#date	>> demo_output.txt; 
date
...
##  grep WER exp/tri3/decode*/wer*
  date
echo "" >> demo_output.txt; 
egrep '^demo' exp/tri3/$decode_dir1/log/decode.1.log | cut -c 7- >> demo_output.txt
echo "" >> demo_output.txt; 
#echo "tri3" >> demo_output.txt
#date	>> demo_output.txt; 
fi
...
echo "" >> demo_output.txt; 
egrep '^demo' exp/sgmm2_4/$decode_dir1/log/decode_pass1.1.log | cut -c 7- >> demo_output.txt
echo "" >> demo_output.txt; 
#echo "sgmm" >> demo_output.txt
#date	>> demo_output.txt; 
fi
...
echo "" >> demo_output.txt; 
egrep '^demo' exp/DNN_tri3_aligned/$decode_dir1/log/decode.1.log | cut -c 7- >> demo_output.txt
echo "" >> demo_output.txt; 
echo "DNN" >> demo_output.txt
#date	>> demo_output.txt; 
  date
